...
export const baseCheckUnit = (ent: Unit) =>
	ent.IsAlive && !ent.HasNoCollision && ent.HasMoveCapability(DOTAUnitMoveCapability.DOTA_UNIT_CAP_MOVE_GROUND)

export const checkControllable = (ent: Unit) => baseCheckUnit(ent) && ent.IsControllable

export const Controllables = () =>
	EntityManager.GetEntitiesByClass(Unit).filter(unit => baseCheckUnit(unit) && checkControllable(unit))

export const getCenterDirection = (units: Entity[]) => Vector3.GetCenterType(units, unit => unit.InFront(350))
