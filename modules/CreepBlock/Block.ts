...
import { ArrayExtensions, Creep, EntityManager, ExecuteOrder, GameSleeper, GameState, LocalPlayer, Menu, RendererSDK, Tower, Unit, VKeys, VMouseKeys } from "github.com/octarine-public/wrapper/index"
...
Key.OnValue(caller => {
	...
	const isPressed = caller.is_pressed
...
export function KeyDown(key: VKeys): boolean {
	if (Key.assigned_key === key && KeyStyle.selected_id !== 1) {
		turnStateBlock = true
		return false
	}
	return true
...
function GetGroupsCreeps() {
	...
	creeps.forEach(creep => {
		const group = creeps.filter(creepNear => creep.IsInRange(creepNear, 500)
